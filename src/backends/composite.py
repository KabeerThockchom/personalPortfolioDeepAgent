...
from .protocol import BackendProtocol, WriteResult, EditResult
from .state import StateBackend
from .utils import FileInfo, GrepMatch


class CompositeBackend:
    """Composite backend that routes file operations to different backends based on path prefix.

    This enables hybrid storage where different directories use different backends:
    - /working/ → StateBackend (ephemeral, within thread)
    - /memories/ → StoreBackend (persistent, across sessions)
    - /reports/ → FilesystemBackend (real disk files)
    """
